...
def fetch_games(url, max_games=10):
            ...
            if w_elo < 1500 and b_elo < 1500 and is_blitz(time_control):
                ...
                if white_tilts or black_tilts:
                    ...
                    for t in white_tilts:
                        print(
                            f"  WHITE TILT! Blunder: {t['blunder_number']}. {t['blunder_move']} (Eval drop: {t['blunder_eval_drop']})"
                        )
                        print(
                            f"    -> Followed by: {[str(m['move_number']) + '. ' + m['move_san'] + ' (' + str(m['time_spent']) + 's)' for m in t['tilt_sequence']]}"
                        )

                    for t in black_tilts:
                        print(
                            f"  BLACK TILT! Blunder: {t['blunder_number']}. {t['blunder_move']} (Eval drop: {t['blunder_eval_drop']})"
                        )
                        print(
                            f"    -> Followed by: {[str(m['move_number']) + '. ' + m['move_san'] + ' (' + str(m['time_spent']) + 's)' for m in t['tilt_sequence']]}"
                        )
    ...
    if analyzed_count > 0:
        ...
        print(
            f"Done. Found {matched} matching games out of {analyzed_count} analyzed games ({percentage:.2f}%)."
        )
