            ...
            if current_eval is not None:
                ...
                if (move_num >= 12) and (is_white_turn == player_color):
                    ...
                    if is_traumatic_blunder(p_prev_eval, p_curr_eval):
                        ...
                        if did_opponent_cancel_blunder(opponent_node, current_eval, is_white_turn):
                            # Skip this blunder
                            prev_eval = current_eval
                            node = next_node
                            is_white_turn = not is_white_turn
                        ...
                        player_times = white_move_times if is_white_turn else black_move_times
                        avg_time = sum(player_times) / len(player_times) if player_times else 10.0
                        ...
                        for _ in range(3):
                            ...
                            tilt_move_data = analyze_tilt_move(temp_node, speed_threshold, is_white_turn)
                        ...
                        if len(tilt_sequence) >= 2:
                            has_significant_drop = any(m['eval_diff'] < -100 for m in tilt_sequence)
                                ...
                                if seq_end_eval is not None:
                                    ...
                                    if seq_diff < -300:
                                        ...
                                        tilt_events.append({
                                            ...
                                            'avg_time_before_blunder': round(avg_time, 2)
                                        })
